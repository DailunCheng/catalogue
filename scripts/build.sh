...
$DOCKER_CMD build -t ${REPO}-dev $BUILD_DIR/docker/catalogue;
$DOCKER_CMD create --name catalogue ${REPO}-dev;
$DOCKER_CMD cp catalogue:/app/main $BUILD_DIR/docker/catalogue/app;
...
$DOCKER_CMD build \
  ...
  -t ${REPO}:${COMMIT} \
  -f $BUILD_DIR/docker/catalogue/Dockerfile-release $BUILD_DIR/docker/catalogue;
